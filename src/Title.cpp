...
void cTitle::Init() {
	...
	m_menu[E_TITLE_MENU] = { {/*左上y*/600,/*左上x*/450,/*右下x*/750,/*右下y*/700,"../resource/img/Button_Start.png" },E_SCENE_MENU };
	m_menu[E_TITLE_END]  = { {/*左上y*/600,/*左上x*/770,/*右下x*/1070,/*右下y*/700,"../resource/img/Button_End.png" }, E_SCENE_END };
	for (int i = 0; i < E_TITLE_MAX; i++) {
		m_btn[i].Init(m_menu[i].image.rect,m_menu[i].image.filePath.c_str());
	}

	//画像の初期化
	m_image[E_BACK_GROUND] = { 0,0,1280,720,"../resource/img/TitleBG.png"};
...
void cTitle::Draw() {
	...
	for (int i = 0; i < E_IMAGE_MAX; i++) {
		DrawExtendGraph(m_image[i].rect.left, m_image[i].rect.top,
			m_image[i].rect.right, m_image[i].rect.bottom, m_image[i].handle, FALSE);
	}
